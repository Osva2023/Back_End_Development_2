const regionSchema = require("../../shared/db/mongodb/region_Schema.js");
const agentSchema = require("../../shared/db/mongodb/agent_Schema.js");
...
const createRegion = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({ message: "Region created successfully", data: regionCheck });
  } catch (error) {
    console.error(error);
...
const displayRegions = async (req, res) => {
  try {
    ...
    const regionmanager = await agentSchema.find({
      region: regionID,
      position: "manager",
    });
    ...
    sortedAgents = sortedAgents.filter(
      (agent) => agent.region.toString() === regionID
    );
    ...
    const topAgentsNames = topAgents.map(
    ...
    return res.status(200).json({
      ...
      total_sales: totalSales[0].totalSales,
      manager: managerFullName,
      top_agents: topAgentsNames,
    });
  } catch (error) {
    console.error(error);
    return res.status(500).send({ message: "Internal server error" });
...
const displayAllStar = async (req, res) => {
  try {
    const pipeline = [
      ...
      { $sort: { "topAgent.sales": -1 } },
      {
        $project: {
          _id: 0,
          agent: {
            $concat: ["$topAgent.first_name", " ", "$topAgent.last_name"],
          },
        },
      },
    ];
    ...
    if (topAgentsByRegion.length === 0) {
      return res
        .status(404)
        .send({ message: "Top agents not found for any region" });
    }

    return res.status(200).json({
      "All Stars Agents: ": topAgentsByRegion.map((result) => result.agent),
    });
  } catch (error) {
    console.error(error);
    return res.status(500).send({ message: "Internal server error" });
